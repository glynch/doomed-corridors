...
into an immutable, renderer-independent model without initializing graphics or
audio. The first playable vertical slice is documented in
...
32 map markers, decodes `MAP01`, and reports its map-record counts.
...
UDMF and Hexen maps explicitly. Generated imported data will be treated as a
cache, so deleting and rebuilding it never loses authored state. The same
