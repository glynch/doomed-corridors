...
headless launcher that validates it, and a bounded WAD reader that verifies and
indexes the pinned Freedoom Phase 2 source without initializing graphics or audio.
The first playable vertical slice is documented in
...
32 map markers, and confirms that `MAP01` is present.

## Project definition and WAD source

The manifest identifies `assets/freedoom2.wad` as the authoritative game-data
source and `MAP01` as the startup target. Generated imported data will be treated
as a cache, so deleting and rebuilding it never loses authored state. The same
