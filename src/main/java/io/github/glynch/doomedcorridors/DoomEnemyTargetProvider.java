...
final class DoomEnemyTargetProvider implements ComponentReferenceBinder {
    private Entity player;

    /** Creates an unbound target which becomes usable after descriptor reference binding. */
    DoomEnemyTargetProvider() {
    ...
    public void bindReferences(ComponentReferenceResolver references) {
        player = Objects.requireNonNull(references, "references")
                .entity(DoomedCorridorsDescriptors.PLAYER_TARGET_PROPERTY);
    }
    ...
    DoomDamageable damageable() {
        return player().capability(DoomedCorridorsDescriptors.DAMAGEABLE_CAPABILITY, DoomDamageable.class)
